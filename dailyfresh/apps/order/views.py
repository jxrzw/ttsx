...
class OrderPlaceView(LoginRequireMixin,View):
    ...
    def post(self,request):
        ...
        user = request.user
        # 获取参数sku_ids
        sku_ids = request.POST.getlist('sku_ids') # [7,2]
...
#前端传递的参数，地址id(addr_id) 支付方式(pay_method) 用户要购买的商品id字符床（sku_ids）
    ...
    def post(self,request):
        ...
        try:
            ...
            order = OrderInfo.objects.create(order_id=order_id,
                                     ...
                                     total_price=total_price,
                                     transit_price=transit_price)
            # todo:用户的订单中有几个商品，需要向de_order_goods表中加入几条记录
            conn = get_redis_connection('default')
            cart_key = 'cart_%d'%user.id
...
class OrderPayView(View):
    ...
    def post(self,request):
        ...
        #调用其支付接口
        #电脑网站支付,需要跳转到https://apenapi.alipaydev.com/gateway.do? + order_string
